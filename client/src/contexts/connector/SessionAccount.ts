...
import {addAddressPadding, Call, CallData, InvokeFunctionResponse, WalletAccount} from "starknet";
    ...
    async execute(calls: Call | Call[]): Promise<InvokeFunctionResponse> {
        try {
            const res = await this.controller.executeFromOutside(
                this.normalizeCalls(calls),
            );
            return res;
        } catch (e) {
            return this.controller.execute(this.normalizeCalls(calls));
        }
    }

    private normalizeCalls(calls: Call | Call[]) {
        return this.toArray(calls).map((call) => {
            return {
